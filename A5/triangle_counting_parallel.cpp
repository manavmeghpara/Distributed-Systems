...
void  triangleCountParallelStrategy1(Graph &g, int world_rank, int world_size ){
  long *buffer;
  double communication_time;
  timer t, c;
  if(world_rank == ROOT_PROCESS){
    buffer = new long[world_size];
    t.start();
  }
...
void  triangleCountParallelStrategy2(Graph &g, int world_rank, int world_size ){
  double communication_time;
  timer t, c;
  if(world_rank == ROOT_PROCESS){
    t.start();
...
int main(int argc, char *argv[])
{
    ...
    if(strategy == 1)
      triangleCountParallelStrategy1(g, world_rank, world_size);
    else if(strategy ==2)
      triangleCountParallelStrategy2(g, world_rank, world_size);
