...
void pageRankParallelStrategy1(Graph &g, int max_iters, int world_rank, int world_size){
    PageRankType *pr_curr = new PageRankType[g.n_];
    PageRankType *pr_next = new PageRankType[g.n_];
    int *size_partition = new int[world_size];
    int *size_each_process;
    ...
    if(world_rank == ROOT_PROCESS){
        t.start();
        size_each_process = new int[world_size];
    }
    ...
    if(world_rank == ROOT_PROCESS){
        ...
        for (int i=0; i<world_size; i++){
            if(i<world_size-1)
                size_each_process[i] = size_partition[i+1]- size_partition[i];
            else
                size_each_process[i] = g.n_ - size_partition[i];
        }
    }
    else{
    ...
    for (int iter = 0; iter < max_iters; iter++)
    {
        ...
        c.start();
        if(world_rank == ROOT_PROCESS){
            MPI_Reduce(MPI_IN_PLACE, pr_next, g.n_, PAGERANK_MPI_TYPE, MPI_SUM, ROOT_PROCESS, MPI_COMM_WORLD);
            MPI_Scatterv(pr_next, size_each_process  , size_partition, PAGERANK_MPI_TYPE, MPI_IN_PLACE, 0, PAGERANK_MPI_TYPE, ROOT_PROCESS, MPI_COMM_WORLD );

        }
        else{
            MPI_Reduce(pr_next, pr_next, g.n_, PAGERANK_MPI_TYPE, MPI_SUM, ROOT_PROCESS, MPI_COMM_WORLD);
            MPI_Scatterv(MPI_IN_PLACE, 0  , size_partition, PAGERANK_MPI_TYPE, &pr_next[size_partition[world_rank]], (end_vertex-start_vertex), PAGERANK_MPI_TYPE, ROOT_PROCESS, MPI_COMM_WORLD );
        }
    ...
    MPI_Reduce(&local_sum, &total_sum, 1, PAGERANK_MPI_TYPE, MPI_SUM, ROOT_PROCESS, MPI_COMM_WORLD);
...
void pageRankParallelStrategy2(Graph &g, int max_iters, int world_rank, int world_size){
    PageRankType *pr_curr = new PageRankType[g.n_];
    PageRankType *pr_next = new PageRankType[g.n_];
    int *size_partition = new int[world_size];
    int *size_each_process;
    ...
    for (int iter = 0; iter < max_iters; iter++)
    {
        ...
        for (int i = 0; i < world_size; i++){
            ...
            if(i==world_rank)
                MPI_Reduce(MPI_IN_PLACE, &pr_next[size_partition[i]], (count) , PAGERANK_MPI_TYPE, MPI_SUM, i, MPI_COMM_WORLD);
            else
                MPI_Reduce(&pr_next[size_partition[i]], &pr_next[size_partition[i]], (count) , PAGERANK_MPI_TYPE, MPI_SUM, i, MPI_COMM_WORLD);
        }
        communication_time += c.stop();
        // --- synchronization phase 1 ends ---
...
int main(int argc, char *argv[])
{
    ...
    if(strategy == 1)
        pageRankParallelStrategy1(g, max_iterations, world_rank, world_size);
    if(strategy == 2)
        pageRankParallelStrategy2(g, max_iterations, world_rank, world_size);
